...
LOG_FILE = f"{LOG_DIR}/{_mode_tag}_long_short_log.csv"
STATUS_FILE = f"{STATUS_DIR}/btc_regime_long_short.csv"
BLACKLIST_FILE = f"{STATUS_DIR}/dynamic_blacklist_long_short.json"
...
SHORT_ENTRY_REGIMES = frozenset([-2, -3])
...
def sim_open_short(symbol, amount, price):
    """
    模擬開空（與多單同一套簡化保證金：鎖定 notional+fee，平倉時釋放並結算 PnL）
    """
    global sim_balance, sim_trade_count
    fee = amount * price * FEE_RATE
    cost = amount * price + fee
    if sim_balance < cost:
        logger.warning(f"🔵 [SIM] {symbol} 餘額不足(空) (需 {cost:.2f}, 有 {sim_balance:.2f})")
        return 0, 0
    sim_balance -= cost
    sim_trade_count += 1
    logger.info(f"🔵 [SIM] OPEN SHORT {symbol} | 數量:{amount} @ {price:.4f} | 費用:{fee:.4f} | 餘額:{sim_balance:.2f}")
    return amount, price
...
def sim_close_short(symbol, amount, price):
    """模擬平空：上漲虧損、下跌盈利。"""
    global sim_balance, sim_total_pnl, sim_equity, sim_trade_count
    if symbol not in sim_positions:
        logger.warning(f"🔵 [SIM] {symbol} 找不到持倉(空)，無法平倉")
        return 0.0
    pos = sim_positions[symbol]
    entry_price = pos['entry_price']
    fee = amount * price * FEE_RATE
...
def sim_get_positions():
    ...
    for symbol, pos in sim_positions.items():
        ...
        result.append({
            ...
            'info': {'side': 'Sell' if is_short else 'Buy'},
    ...
    for symbol, pos in sim_positions.items():
        try:
            ...
            if pos.get('side', 'long') == 'short':
                unrealized += (pos['entry_price'] - curr_p) * pos['amount']
            else:
...
def process_native_exit_log(symbol, pos, position_type='long'):
    ...
    if SIMULATION_MODE:
        ...
        is_short = pos.get('side') == 'short' or position_type == 'short'
        if is_short:
...
def apply_lee_ready_short_logic(symbol):
    """
    與多單對稱：尋找淨主動賣壓、加速向下、盤口偏賣，僅在 regime -2/-3 掃描時呼叫。
    """
    try:
        trades = exchange.fetch_trades(symbol, limit=200)
        if not trades: return 0, 0, False
        ...
        if is_strong and imbalance > 0.1:
            is_strong = False
            print(f"⚠️ {symbol} 假跌陷阱！取消做空！")

        return short_window_flow, df['price'].iloc[-1], is_strong
    except Exception as e:
        print(f"⚠️ LR Short Logic Error [{symbol}]: {e}")
        return 0, 0, False


def check_flow_health_short(symbol):
    """空單離場用：偵測主動買盤回補/軋空風險（與多單 check_flow_health 符號相反）。"""
    try:
        trades = exchange.fetch_trades(symbol, limit=100)
        if not trades or len(trades) < 50: return None
        ...
        if accel_z > 2.0 and recent_25_flow > 0:
            try:
                ...
                imbalance = (bids_vol - asks_vol) / (bids_vol + asks_vol) \
                    if (bids_vol + asks_vol) > 0 else 0
                if imbalance > 0.15:
                    return "Flow Deceleration (Downtrend Died)"
            except:
                pass
        return None
    except:
...
def sync_positions_on_startup():
    ...
    try:
        ...
        for p in live_symbols:
            ...
            if side in ['long', 'buy'] or info_side in ['buy', 'long']:
            ...
            elif side in ['short', 'sell'] or info_side in ['sell', 'short']:
                entry_price = float(p.get('entryPrice', 0))
                amount = float(p.get('contracts', 0) or p.get('size', 0))
                sl_p = float(p.get('stopLoss', 0))
                tp_p = float(p.get('takeProfit', 0))
                ...
                if tp_p == 0:
                    tp_p = float(exchange.price_to_precision(symbol,
                                                             entry_price - (TP_ATR_MULT * atr)))
                is_be = True if (0 < sl_p < entry_price) else False
                positions[symbol] = {
                    'amount': amount, 'entry_price': entry_price,
                    'tp_price': tp_p, 'sl_price': sl_p,
                    'is_breakeven': is_be, 'atr': atr, 'max_pnl_pct': 0.0,
                    'entry_time': time.time(), 'side': 'short',
                }
                recovered_count += 1
                print(f"✅ 尋回孤兒空單: {symbol} | 入場:{entry_price} | 保本:{is_be}")
        print(f"🔄 同步完成！共尋回 {recovered_count} 筆倉位。")
    except Exception as e:
...
def manage_long_positions(regime=None):
    """
    多/空倉位管理（[FIX-SIM] Sim/Live 統一介面）

    空單由 positions[s]['side'] == 'short' 標記；出場/移損邏輯與多單對稱。
    """
    try:
            ...
            if s not in positions:
                ...
                if side in ['long', 'buy'] or info_side in ['buy', 'long']:
                    ...
                    positions[s] = {
                        ...
                        'tp_price': tp_p, 'sl_price': sl_p,
                        'is_breakeven': is_be, 'atr': atr, 'max_pnl_pct': 0.0,
                        'entry_time': real_entry_time, 'side': 'long',
                    }
                    ...
                    if tp_p == 0:
                        tp_p = float(exchange.price_to_precision(
                            s, entry_p - (TP_ATR_MULT * atr)))
                    is_be = True if (0 < sl_p < entry_p) else False
                    positions[s] = {
                        'amount': amt, 'entry_price': entry_p,
                        'tp_price': tp_p, 'sl_price': sl_p,
                        'is_breakeven': is_be, 'atr': atr, 'max_pnl_pct': 0.0,
                        'entry_time': real_entry_time, 'side': 'short',
                    }
                    print(f"🚨 [自癒] 接管孤兒空單: {s} | 入場:{entry_p}")
        ...
        for s in list(positions.keys()):
            try:
                ...
                adverse_for_short = rs0 in (1, 2)

                pnl_pct = (
                    (pos['entry_price'] - curr_p) / pos['entry_price'] if is_short
                    else (curr_p - pos['entry_price']) / pos['entry_price']
                )
                ...
                if pos['is_breakeven']:
                    if is_short:
                        if regime and adverse_for_short:
                    ...
                    if time_held > 1200:
                        if is_short:
                            ...
                            if flow_status == "Flow Reversal (Short Squeeze Detected)":
                                price_bounce = (curr_p - pos['entry_price']) / pos['entry_price']
                                if price_bounce > 0.005:
                                    exit_reason = flow_status
                                else:
                                    print(f"⚠️ {s} Short Squeeze 信號但價格未確認"
                                          f"（反彈 {price_bounce * 100:.2f}% < 0.5%），忽略")
                            elif flow_status == "Flow Deceleration (Downtrend Died)":
                                if not pos.get('deceleration_detected'):
                                    pos['deceleration_detected'] = True
                                    print(f"⚠️ {s} 低位反彈偵測！啟動防禦標記！")
                        else:
                ...
                if exit_reason:
                    ...
                    if is_short:
                        if SIMULATION_MODE:
                            ...
                            ioc_pnl = sim_close_short(s, pos['amount'], ioc_price)
                            if s in sim_positions:
                                del sim_positions[s]
                        else:
                            ioc_price = get_3_layer_avg_price(s, 'asks') or curr_p
                            try:
                                exchange.create_order(s, 'limit', 'buy', pos['amount'],
                                                      ioc_price,
                            ...
                            ioc_pnl = round(
                                (pos['entry_price'] - ioc_price) * pos['amount'] - entry_fee - exit_fee, 4
                            )
                            _positions_cache['ts'] = 0
                        exit_action = 'SHORT_EXIT'
                    else:
                        if SIMULATION_MODE:
                            ...
                            _positions_cache['ts'] = 0
                        exit_action = 'LONG_EXIT'

                    log_to_csv({
                        'symbol': s, 'action': exit_action, 'price': curr_p,
...
def execute_live_long(symbol, net_flow, current_price, is_strong,
    ...
    if not SIMULATION_MODE:
        ...
        sim_positions[symbol] = {
            ...
            'tp_price': tp_p, 'sl_price': sl_p, 'entry_time': time.time(), 'side': 'long',
        }
...
def execute_live_short(symbol, net_flow, current_price, is_strong,
                       atr, is_volatile, regime=None, position_multiplier=1.0):
    """
    空單入場（僅由主迴圈在 regime ∈ SHORT_ENTRY_REGIMES 時呼叫）。
    Sim/Live 介面與 execute_live_long 對稱。
    """
    _r = regime or {}
    regime_signal_tag = _r.get('regime_signal', 0)
    adx_tag = round(_r.get('mean_adx', 0), 2)
    ...
    if regime_signal_tag not in SHORT_ENTRY_REGIMES:
        return
        ...
        try:
            ...
            if symbol in live_syms:
                for p in live_pos:
                    ...
                    info_side = p.get('info', {}).get('side', '').lower()
                    is_live_short = (
                        side in ['short', 'sell'] or info_side in ['sell', 'short'])
                    if not is_live_short:
                        logger.warning(
                            f"⚠️ [DUPCHECK] {symbol} 交易所已有非空倉，拒絕開空")
                        return
                    entry_p = float(p.get('entryPrice', 0))
                    amt = float(p.get('contracts', 0) or p.get('size', 0))
                    atr_v, _ = get_market_metrics(symbol)
                    ...
                    sl_p = float(p.get('stopLoss') or 0) or float(
                        exchange.price_to_precision(
                            symbol, entry_p + SL_ATR_MULT * atr_v))
                    tp_p = float(p.get('takeProfit') or 0) or float(
                        exchange.price_to_precision(
                            symbol, entry_p - TP_ATR_MULT * atr_v))
                    positions[symbol] = {
                        'amount': amt, 'entry_price': entry_p,
                        'tp_price': tp_p, 'sl_price': sl_p,
                        'is_breakeven': sl_p < entry_p,
                    ...
                    print(f"🔄 [DUPCHECK] 已補回 {symbol} 空單至本地 positions dict")
                    break
                return
        except Exception as e:
            logger.warning(f"⚠️ [DUPCHECK] {symbol} 交易所倉位查詢失敗，繼續執行: {e}")
    ...
    if len(positions) >= MAX_CONCURRENT_POSITIONS:
        logger.debug(f"⛔ {symbol} 倉位上限 {MAX_CONCURRENT_POSITIONS}，拒絕入場(空)")
        return
    ...
    recent_sl_times[:] = [t for t in recent_sl_times
                          if now - t < CASCADE_SL_WINDOW]
    if len(recent_sl_times) >= CASCADE_SL_TRIGGER:
        logger.debug(f"⛔ {symbol} Cascade SL 保護中，拒絕入場(空)")
        return
    ...
    if not SIMULATION_MODE:
        try:
            ...
            if "110043" not in str(e):
                if "110026" in str(e):
                    return
                logger.warning(f"⚠️ {symbol} 槓桿異常(空): {e}")

    if SIMULATION_MODE:
        actual_amount, actual_price = sim_open_short(symbol, amount, ioc_p)
        if actual_amount == 0:
            print(f"⏩ [SIM] {symbol} 餘額不足(空)，跳過。")
            return
    else:
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ {symbol} 空單訂單確認失敗，備用同步: {e}")
                time.sleep(0.5)
                for p in exchange.fetch_positions(params={'category': 'linear'}):
                    if (p['symbol'] == symbol and
                            float(p.get('contracts', 0) or p.get('size', 0)) > 0):
                        ...
                        break
            if actual_amount == 0:
                print(f"⏩ {symbol} 空單 IOC 未成交，撤單退出。")
                cancel_all_v5(symbol)
                return
        except Exception as e:
    ...
    if (actual_price - tp_p) / actual_price < 0.003:
        print(f"🟡 放棄做空 [{symbol}]: 利潤空間太細！"
              + (" [SIM 無需平倉]" if SIMULATION_MODE else ""))
        if SIMULATION_MODE:
            global sim_balance
            refund = actual_amount * actual_price
            sim_balance += refund
            logger.info(f"🔵 [SIM] {symbol} 退還本金(空) {refund:.4f}（利潤空間太細）")
        else:
            try:
                exchange.create_market_buy_order(
                    symbol, actual_amount, {'reduceOnly': True})
    ...
    if not SIMULATION_MODE:
        try:
            ...
            print(f"✅ {symbol} 空單 TP/SL | TP:{tp_p} | SL:{sl_p}")
        except Exception as e:
            logger.warning(f"⚠️ {symbol} 空單 TP/SL 異常: {e}")
        _positions_cache['ts'] = 0
    else:
        print(f"🔵 [SIM] {symbol} 虛擬空單 TP:{tp_p} | SL:{sl_p}")
        sim_positions[symbol] = {
            'amount': actual_amount, 'entry_price': actual_price,
            'tp_price': tp_p, 'sl_price': sl_p, 'entry_time': time.time(), 'side': 'short',
        }
    ...
    if TELEGRAM_ENABLED:
        try:
            telegram_notifier.send_trade_alert(
                ...
                price=actual_price,
                amount=actual_amount,
                reason=f"趨勢/熊市空頭 | ADX:{adx_tag} | Score:{score_tag}"
            )
        except Exception as e:
            logger.warning(f"⚠️ Telegram通知發送失敗(空): {e}")
...
def main():
    ...
    print(f"🚀 AI 實戰 V3 Long/Short [{mode_label}] 啟動...")
    ...
    while True:
        try:
            ...
            if curr_t - last_scout_time > SCOUTING_INTERVAL:
                ...
                can_short_entry = regime_signal in SHORT_ENTRY_REGIMES

                # Sensor B: 每次 scout 更新 regime 方向歷史
                ...
                if is_long_signal:
                    ...
                    if _current_state != _last_brake_state:
                        if regime_signal == -1:
                            print("🚫 MR空頭(-1)：暫停多單，不開空（僅 -2/-3 可做空）")
                ...
                print(f"⏳ {'[SIM]' if SIMULATION_MODE else ''} 多空巡邏 | "
